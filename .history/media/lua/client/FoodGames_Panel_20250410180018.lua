...
function FoodGamesPanel:createChildren()
    ...
    self.CalSlider = ISSliderPanel:new(self.width / 2 - 400 / 2, self.height / 2, 400, 20, self,  nil)
    ...
    local calData = self.player:getModData()['FoodGames']['consumedCalories']
    --CalSlider.valueLabel = false;
    --CalSlider.customData = calData;

    --_min, _max, _step, _shift
    self.CalSlider:setValues(0.0, 99999, 0.1, 1)
	self.CalSlider:setCurrentValue(calData)

    self:addChild(CalSlider);
