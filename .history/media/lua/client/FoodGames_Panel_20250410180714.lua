...
function FoodGamesPanel:createChildren()
    ...
    self.CalSlider = ISSliderPanel:new(self.width / 2 - 200 / 2, self.height / 2, 200, 40, self, nil)
    ...
    local calData = self.player:getModData()['FoodGames']['consumedCalories'] or 0
    --CalSlider.valueLabel = false;
    --CalSlider.customData = calData;

    --_min, _max, _step, _shift
    self.CalSlider:setValues(0.0, 99999, 0.1, 1, false)
	self.CalSlider:setCurrentValue(calData)
    self.CalSlider:setHeight(40)

    self.CalSlider:setEditable(false);
    self.CalSlider:setMinMax(0, 99999);
