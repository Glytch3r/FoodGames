...
function FoodGamesPanel:prerender()
    ...
    local consumedCalories =  self.player:getModData()['FoodGames']['consumedCalories']
    if tonumber(self.CalSlider:getCurrentValue()) ~= tonumber(consumedCalories) then
        self.CalSlider:setValue( tonumber(consumedCalories))
    ...
    local caption = tostring(consumedCalories)
    if getCore():getDebug() then
        caption = tostring(self.CalSlider:getCurrentValue())
    end
    self:drawTextCentre(caption, self:getWidth() / 2, 32, 0.76, 0.59, 0.11, 1, UIFont.Large);
