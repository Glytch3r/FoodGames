...
function FoodGamesPanel:prerender()
    ...
    local consumedCalories =  tonumber(self.player:getModData()['FoodGames']['consumedCalories'])
    local caption = tostring(consumedCalories)
    if tonumber(self.CalSlider:getCurrentValue()) ~= consumedCalories then
        self.CalSlider:setValue(consumedCalories)
    end
