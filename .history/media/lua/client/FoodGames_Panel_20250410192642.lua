...
function FoodGamesPanel:prerender()
    ...
    local foodData = modData and modData['FoodGames']
    local consumed = foodData and tonumber(foodData['consumedCalories']) or 0

    if consumed < 0 then consumed = 0 end
    if consumed > 99999 then consumed = 99999 end

    local currentSliderVal = self.CalSlider:getCurrentValue()
    if math.abs(currentSliderVal - consumed) > 0.01 then
        self.CalSlider:setValue(consumed)
    end

    self:drawTextCentre(tostring(consumed), self:getWidth() / 2, 32, 0.76, 0.59, 0.11, 1, UIFont.Large)
end
