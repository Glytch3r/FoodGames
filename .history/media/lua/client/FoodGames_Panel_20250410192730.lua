...
function FoodGamesPanel:prerender()
    ...
    if self.player and self.player:getModData() then
        local foodGamesData = self.player:getModData()['FoodGames']
        if foodGamesData and type(foodGamesData['consumedCalories']) == "number" then
            calories = foodGamesData['consumedCalories']
        else
            -- fallback / sanitize corrupted data
            calories = tonumber(foodGamesData and foodGamesData['consumedCalories']) or 0
        end
    end

    -- clamp the value
    if calories < 0 then calories = 0 end
    if calories > 99999 then calories = 99999 end

    -- only update if out of sync
    if self.CalSlider and math.abs(self.CalSlider:getCurrentValue() - calories) > 0.01 then
        pcall(function() self.CalSlider:setValue(calories) end)
    end

    self:drawTextCentre(tostring(math.floor(calories)), self:getWidth() / 2, 32, 0.76, 0.59, 0.11, 1, UIFont.Large)
end
